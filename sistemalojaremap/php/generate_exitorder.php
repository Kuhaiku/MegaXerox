        ...
        document.getElementById('gerarOrdem').addEventListener('click', function () {
            ...
            .then(data => {
                ...
                if (data.toLowerCase().includes("sucesso")) {
                    // Lógica de Redirecionamento após a Impressão
                    if (window.matchMedia) {
                        // Usa mediaQuery para detectar o fechamento do print
                        const mediaQueryList = window.matchMedia('print');
                        mediaQueryList.addListener(function(mql) {
                            if (!mql.matches) {
                                // Ocorre quando a impressão é fechada (cancelada ou concluída)
                                window.location.href = '../index.html'; 
                            }
                        });
                        window.print();
                        // Fallback (e para o caso do navegador não ter o evento afterprint)
                        setTimeout(function() {
                            window.location.href = '../index.html'; 
                        }, 500); 
                    } else {
                        // Método simples para navegadores mais antigos
                        window.print();
                        setTimeout(function() {
                            window.location.href = '../index.html'; 
                        }, 500);
